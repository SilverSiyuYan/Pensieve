...
allowed_origins = [
    origin.strip() for origin in os.getenv("CORS_ALLOWED_ORIGINS", "http://localhost:8080").split(",") if origin.strip()
]
app.add_middleware(
    CORSMiddleware,
...
def store_memory(payload: MemoryStoreRequest, current_user: CurrentUser) -> dict[str, Any]:
    return _store_memory(str(current_user["id"]), payload.content, payload.tags, payload.category)
...
def auto_memory(payload: AutoMemoryRequest, current_user: CurrentUser) -> dict[str, Any]:
    ...
    if intent_result["intent"] == "store":
        result = _store_memory(user_id, intent_result["extracted_content"], intent_result["extracted_tags"], "未分类")
...
def get_memories(
    ...
    return list_memories(str(current_user["id"]), limit=limit, offset=offset, category_filter=category)
